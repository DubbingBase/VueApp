    ...
    <div class="bg-gray-900 p-6 rounded-2xl border border-gray-800 flex justify-between items-center shadow-xl">
      <div>
        <h3 class="text-xl font-bold text-white flex items-center gap-2">
          ...
          {{ isEditMode ? $t('projectEditor.editMovieProject') : $t('projectEditor.createMovieProject') }}
        </h3>
      ...
      >
        <span>{{ tmdbMovieId ? $t('projectEditor.backToMovie') : $t('projectEditor.backHome') }}</span>
      </NuxtLink>
    </div>

    <!-- Navigation Tabs -->
    <div v-if="tmdbMovieId" class="flex flex-wrap gap-2 pb-2 border-b border-gray-800">
      <NuxtLink
    ...
    <form v-else @submit.prevent="saveMovieProject" class="space-y-6">
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-2xl p-6 space-y-5 h-fit shadow-xl">
          ...
          <!-- Content ID / TMDB ID -->
          <div class="space-y-1">
            <label class="text-xs font-semibold text-gray-400 uppercase tracking-wider">{{ $t('projectEditor.tmdbMovieId') }}</label>
...
const tmdbMovieId = computed(() => (movieIdParam && movieIdParam !== "new") ? Number(movieIdParam) : null);
...
const searchActors = (query: string) => {
  ...
  filteredTmdbCast.value = tmdbCast.value.filter(c => 
    ...
    (c.character && c.character.toLowerCase().includes(query.toLowerCase()))
  ).slice(0, 50);
};
...
const saveMovieProject = async () => {
  ...
  try {
    ...
    if (crewJobs.length > 0) {
      ...
      const { error: insertCrewError } = await supabase.from("dubbing_project_crew").insert(crewPayload as any);
...
const { data: initialData } = await useAsyncData(`movie-edit-${tmdbMovieId.value}-${projectIdParam}`, async () => {
  ...
  if (tmdbMovieId.value) {
    // TMDB metadata
    try {
      const data = await $fetch<any>('/api/movie', { params: { id: tmdbMovieId.value } });
      if (data && data.movie) {
        tmdbData = data.movie;
      }
    } catch (e) {
